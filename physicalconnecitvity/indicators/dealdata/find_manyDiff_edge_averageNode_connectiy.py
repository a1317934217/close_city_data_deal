#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2022/3/24 18:33
# @Author  : wuhao
# @Email   : guess?????
# @File    : find_manyDiffE_eage_averageNode_connectiy.py
import csv
...
def find_city_Counter(edges_add_list):
    ...
    return count
#增加连边计算平均点连通性
def useCity_addEdges_Tocaculate_average_node_connectity(city_name,add_edges,G_contrast):
    """
    ...
    return  format(nx.average_node_connectivity(G_contrast), '.2f')
#减少连边计算平均点连通性
def userCity_removeEdges_Tocaculate_average_node_connectity(city_name,add_edges,G_contrast):
    """
...
def tocsv_mutipul_edges(result_filename):
    ...
    for result_filename_one in result_filename:
        ...
        if result_filename_one[14:15] == "a":
            ...
            with open(average_node_result_Edges_fileFront +"multiple_cityedge_averageNode\\" + result_filename_one[0:9]+"addEdgesValue.csv" , 'w', encoding="utf-8", newline='') as csvfile:
            ...
            with open(average_node_result_Edges_fileFront +"multiple_cityedge_averageNode\\" + result_filename_one[0:9]+"removeEdgesValue.csv" , 'w', encoding="utf-8", newline='') as csvfile:
                writer = csv.DictWriter(csvfile, field_order_move_in)
                writer.writeheader()
                # dict_cite_value = {}
                for city_name_need_run in count_remove.most_common():
                    # print(city_name_need_run[0], city_name_need_run[1])
                    final_value = userCity_removeEdges_Tocaculate_average_node_connectity(city_name_need_run[0],remove_add_list,G_contrast)
                    # dict_cite_value[city_name_need_run] = final_value
                    # sorted(dict_cite_value.items(), key=lambda x: x[1], reverse=True)
                    row = {"city_name": city_name_need_run[0], "indicator_value": final_value, "edgenum": city_name_need_run[1]}
                    writer.writerow(row)
            df = pd.read_csv(average_node_result_Edges_fileFront + "multiple_cityedge_averageNode\\" + result_filename_one[0:9] + "removeEdgesValue.csv")
            df.sort_values(by="indicator_value", ascending=False)
#
