...
def averageedgeconnectivity():
    ...
    listAverageEdgeConnectivity_superNode = []
    for i in range(len(listXData)):
        ...
        try:
            ...
            denominator = len(allNodesList) * len(allNodesList)-1
            AEC_LastValue = 0
            for node_one in range(0, len(allNodesList)):
                for node_two in range(node_one + 1, len(allNodesList)):
                    AEC_EveryValue = len(list(nx.all_simple_paths(G,source=allNodesList[node_one],target=allNodesList[node_two])))
                    AEC_LastValue = AEC_LastValue + AEC_EveryValue
            listAverageEdgeConnectivity.append(AEC_LastValue/denominator)
            listAverageEdgeConnectivity_superNode.append(AEC_LastValue/denominator)
