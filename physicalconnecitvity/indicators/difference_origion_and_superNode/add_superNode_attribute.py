...
def draw_superNode(listData,nodeList):
    # 聚类系数
    average_clustering_origion = []
    average_clustering_superNode = []
    # 边数量
    edge_number_origion = []
    edge_number_superNode = []
    # 密度
    density_origion = []
    density_superNode = []
    # 全局效应
    global_efficiency_origion = []
    global_efficiency_superNode = []
    # 度
    degree_origion = []
    degree_superNode = []
    # 点数量
    edge_origion = []
    edge_superNode = []
    for data_every in listData:
        G_origion = drawpicture(fileNamePath+data_every+ "finalData.csv")
        #这里计算原始网络的参数
        #聚类系数
        average_clustering_origion.append(format(nx.average_clustering(G_origion), '.3f'))
        # 边数量
        edge_number_origion.append(len(G_origion.edges()))
        #密度
        density_origion.append(format(nx.density(G_origion), '.3f'))
        #全球效应
        global_efficiency_origion.append(format(nx.global_efficiency(G_origion), '.3f'))
        #点数量
        edge_origion.append(len(G_origion.nodes()))
        avergerDegree_origion = 0
        for degreeEvery in G_origion.degree():
            avergerDegree_origion = avergerDegree_origion + degreeEvery[1]
        degree_origion.append(format(avergerDegree_origion / 295, '.3f'))
        
        
        for node_add in nodeList:
            G_origion.add_edges_from([(node_add, "超级节点")])
        #这里计算加了超级节点后网络的参数
        # 聚类系数
        average_clustering_superNode.append(format(nx.average_clustering(G_origion), '.3f'))
        # 边数量
        edge_number_superNode.append(len(G_origion.edges()))
        #密度
        density_superNode.append(format(nx.density(G_origion), '.3f'))
        # 全球效应
        global_efficiency_superNode.append(format(nx.global_efficiency(G_origion), '.3f'))
        #度
        avergerDegree_super = 0
        for degreeEvery in G_origion.degree():
            avergerDegree_super = avergerDegree_super + degreeEvery[1]
        degree_superNode.append(format(avergerDegree_super / 295, '.3f'))
        #点
        edge_superNode.append(len(G_origion.nodes()))
    return average_clustering_origion,average_clustering_superNode,edge_number_origion,edge_number_superNode,\
           density_origion,density_superNode,global_efficiency_origion,global_efficiency_superNode,degree_origion,\
           degree_superNode,edge_origion,edge_superNode


average_clustering_origion, average_clustering_superNode,\
edge_number_origion, edge_number_superNode,density_origion, \
density_superNode, global_efficiency_origion, global_efficiency_superNode, \
degree_origion, degree_superNode,edge_origion,edge_superNode= draw_superNode(listXData,nodeList)
print("聚类系数_origion",average_clustering_origion )
print("聚类系数_superNode",average_clustering_superNode)

print("边数量_origion",edge_number_origion )
print("边数量_superNode",edge_number_superNode)

print("密度_origion",density_origion )
print("密度_superNode",density_superNode)

print("全局效应_origion",global_efficiency_origion )
print("全局效应_superNode",global_efficiency_superNode)

print("度_origion",degree_origion )
print("度_superNode",degree_superNode)

print("点_origion",edge_origion )
print("点_superNode",edge_superNode)
