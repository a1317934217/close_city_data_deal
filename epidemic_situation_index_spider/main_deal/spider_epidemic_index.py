...
epidemic_data_file_location = "F:/疫情搜索指数数据/日常维护/"

epidemic_provence_data_file_location = "F:/疫情搜索指数数据/日常维护/31个省数据/"
#376个地市级信息
...
def spider_city():
    """
    第一次处理的爬取   目前在使用下一个return的方法  未改造
    :return:
    """
...
def return_spider(list):
    ...
    for cityname in list:
        try:
            ...
            # url ="https://www.baidu.com/s?ie=utf-8&f=8&rsv_bp=1&tn=baidu&wd={}&oq=%25E4%25B8%258A%25E6%25B5%25B7%25E7%2596%25AB%25E6%2583%2585oq%253D%2526lt%253B%2526gt%253B7%2526lt%253B96%2526lt%253BAB%2526lt%253B%2526gt%253B6%2526lt%253B8%2526lt%253B%2526lt%253B85%2526lt%253B%2526gt%253B6%2526lt%253B90%2526lt%253B9%2526lt%253B%2526lt%253B%2526gt%253B7%2526lt%253BB4%2526lt%253BA2%2526lt%253B%2526gt%253B6%2526lt%253B8%2526lt%253B%2526lt%253B87%2526lt%253B%2526gt%253B6%2526lt%253B95%2526lt%253BB0&rsv_pq=d610099400091b21&rsv_t=9ac1cTtdzhTCgOV3iYhkzNwNZH20%2F%2BVRN3%2FHmXE0dgdCVTfGLfQkcyYIJ2k&rqlang=cn&rsv_dl=tb&rsv_enter=1&rsv_sug3=38&rsv_sug1=13&rsv_sug7=101&rsv_sug2=0&rsv_btype=t&inputT=5819&rsv_sug4=6149".format(cityname)

            url ="https://www.baidu.com/s?ie=UTF-8&wd=疫情指数"
            ...
            try:
                ...
                soup = bs4.BeautifulSoup(needDeal_html, 'html.parser')
                # 获取到div数据块
                divneed = soup.find("div", class_="_aladdin_o4ypw_1 ms-epidemic-exp-san_6pupP pc_63L6z")
                # 字符串拼接取到注释的   疫情搜索比例和 健康指数
                aa = str(divneed).split("s-data:")
                cc = aa[1].split("-->")
                ...
                for i in data_list_needDeal:
                    ...
                    if str(i[:2]) == "12":
                        ...
                        data_list.append(str(year_current) + i.replace(".", ""))
                health_data = jsonData["trendData"]["dayData"]["seriesDatas"][0]
                search_data = jsonData["trendData"]["dayData"]["seriesDatas"][1]
                title_excel = jsonData["title"]
                print(data_list)
...
def simple_deal():
    """
    处理文件名称的显示   获取到的为”xxx疫情指数“的字符串
    :return:
    """

    # 获取到文件名称
    # path1 = r"F:\疫情搜索指数数据\整理之后数据"
    # f = os.listdir(path1)
    # print(f)

    # 处理文件名称
    file_list = []    # 我省略掉了 是读取到的文件名称
    list_new = []
    for i in file_list:
        if "-" in i:
            index1 = i.index("-")
            index2 = i.index(".")
            list_new.append(i[index1 + 1:index2])
        else:
            index1 = i.index(".")
            list_new.append(i[:index1])
    print(list_new)
