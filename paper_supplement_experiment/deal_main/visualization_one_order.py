...
def draw_nine_pic():
    ...
    :return:
    """
    G_one = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                        "石家庄/石家庄一阶/deal_03/20210101_石家庄.csv",First_order)


    G_two = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                        "石家庄/石家庄一阶/deal_03/20210104_石家庄.csv",First_order)
    G_three = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                          "石家庄/石家庄一阶/deal_03/20210106_石家庄.csv",First_order)

    G_four = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                        "石家庄/石家庄一阶/deal_03/20210110_石家庄.csv",First_order)


    G_five = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                        "石家庄/石家庄一阶/deal_03/20210113_石家庄.csv",First_order)
    G_six = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                          "石家庄/石家庄一阶/deal_03/20210114_石家庄.csv",First_order)

    G_seven = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                        "石家庄/石家庄一阶/deal_03/20210206_石家庄.csv",First_order)


    G_eight = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                        "石家庄/石家庄一阶/deal_03/20210226_石家庄.csv",First_order)
    G_nine = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/"
                          "石家庄/石家庄一阶/deal_03/20210324_石家庄.csv",First_order)
...
def get_diff_edge():
    un_Lockdown_file = "F:/封城数据处理/paper_supplement_experiment/data/石家庄/石家庄一阶/deal_03/20210101_石家庄.csv"
    Lockdowm_file = "F:/封城数据处理/paper_supplement_experiment/data/石家庄/石家庄一阶/deal_03/20210113_石家庄.csv"
    un_Lockdown = pd.read_csv(un_Lockdown_file, encoding="utf-8")  # ,encoding="utf-8"
    Lockdowm = pd.read_csv(Lockdowm_file, encoding="utf-8")  # ,encoding="utf-8"
    # 创建处理完的数据csv
    # 表头
    field_order_move_in = ["city_name", 'city_id_name', 'num']
    # 开始写入整理完的数据csv
    with open("F:/封城数据处理/paper_supplement_experiment/data/石家庄/封城前后差异边/diff_one.csv", 'w',
              encoding="utf-8", newline='') as csvfile:
        writer = csv.DictWriter(csvfile, field_order_move_in)
        writer.writeheader()
        for nolockdown in un_Lockdown.iterrows():
            # 取到某一天的所有城市名称
            one = []
            city_name_one = nolockdown[1]['city_name']
            city_id_name_one = nolockdown[1]['city_id_name']
            one.append(city_name_one)
            one.append(city_id_name_one)
            for down in Lockdowm.iterrows():
                two=[]
                city_name_two = down[1]['city_name']
                city_id_name_two = down[1]['city_id_name']
                num = down[1]['num']
                two.append(city_name_two)
                two.append(city_id_name_two)
                if compare_two_str(one,two):
                    row = {"city_name": city_name_two, "city_id_name": city_id_name_two, "num": num}
                    writer.writerow(row)


def noneThing_method():
    un_Lockdown_file = "F:/封城数据处理/paper_supplement_experiment/data/石家庄/石家庄一阶/deal_03/20210101_石家庄.csv"
    Lockdowm_file = "F:/封城数据处理/paper_supplement_experiment/data/石家庄/石家庄一阶/deal_03/20210113_石家庄.csv"
    un_Lockdown = pd.read_csv(un_Lockdown_file, encoding="utf-8")  # ,encoding="utf-8"
    Lockdowm = pd.read_csv(Lockdowm_file, encoding="utf-8")  # ,encoding="utf-8"

    three_one = pd.read_csv("F:/封城数据处理/paper_supplement_experiment/data/石家庄/封城前后差异边/diff_one.csv",
    ...
    for nolockdown in un_Lockdown.iterrows():
        ...
        city_name_one = nolockdown[1]['city_name']
        city_id_name_one = nolockdown[1]['city_id_name']
        aaa.append(city_name_one)
        aaa.append(city_id_name_one)
        one.append(aaa)
    for down in Lockdowm.iterrows():
        bbb = []
        city_name_two = down[1]['city_name']
        city_id_name_two = down[1]['city_id_name']
        bbb.append(city_name_two)
        bbb.append(city_id_name_two)

        one.append(bbb)
    print("长度", len(one), "全部的", one)
    for last in three_one.iterrows():
        ccc = []
        city_name_three = last[1]['city_name']
        city_id_name_three = last[1]['city_id_name']
        ccc.append(city_name_three)
        ccc.append(city_id_name_three)
        two.append(ccc)
    print("长度", len(two), "需要删除的", two)

    for i in two:
        for j in one:
            while compare_two_str(i, j):
                aaa=bbb

    print(set(one).difference(two))  # a中有而b中没有的非常高效！

    # one = ("石家庄","北京") #北京
    # two = ("石家庄","西安")
    # print(compare_two_str(one,two))
    # draw_nine_pic()
    # get_diff_edge()
def outFile():
    last_edge = [['石家庄', '北京'], ['承德', '北京'], ['唐山', '北京'], ['邯郸', '北京'], ['沧州', '北京'], ['沧州', '天津'], ['石家庄', '天津'],
                 ['秦皇岛', '天津'], ['张家口', '天津'], ['邢台', '石家庄'], ['保定', '石家庄'], ['衡水', '石家庄'],
                 ['邯郸', '石家庄'], ['沧州', '石家庄'], ['唐山', '石家庄'], ['廊坊', '石家庄'], ['张家口', '石家庄'],
                 ['廊坊', '唐山'], ['保定', '唐山'], ['邢台', '邯郸'], ['保定', '邯郸'],
                 ['衡水', '邯郸'], ['衡水', '邢台'], ['保定', '邢台'], ['廊坊', '保定'], ['衡水', '保定'],
                 ['张家口', '保定'], ['廊坊', '沧州']]
    ...
    with open("F:/封城数据处理/paper_supplement_experiment/data/石家庄/封城前后差异边/diff_one.csv", 'w',
        ...
        writer.writeheader()
        for i in last_edge:
            row = {"city_name": i[0], "city_id_name": i[1]}
            writer.writerow(row)


def draw_diff_edges_alone():
    G_one = drawpicture("F:/封城数据处理/paper_supplement_experiment/data/石家庄/封城前后差异边/diff_one.csv", First_order)
    plt.subplot(111)
    plt.figure(figsize=(4, 4), dpi=450)
    plt.rcParams['font.sans-serif'] = ['SimHei']
    pos_eight = nx.circular_layout(G_one)
    pos_eight["石家庄"] = (0, 0)
    # plt.title("(h) 2021年2月26日")
    print("第八个子图。边：", len(G_one.edges()), "中心城市度", G_one.degree("石家庄"))
    nx.draw(G_one, pos_eight, node_color="red", with_labels=True, node_size=12)
    plt.show()
if __name__ == '__main__':

    # draw_diff_edges_alone()
    draw_nine_pic()
