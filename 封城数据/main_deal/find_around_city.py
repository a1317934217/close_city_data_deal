...
#西安  作为选择参考的文件位置
reference_file = "F:/封城数据处理/封城数据/main_deal/data/西安/20211210_merge.csv"
#合并迁徙指数位置文件
file_index_out = "F:/封城数据处理/封城数据/main_deal/data/西安/610100_西安_move_out.csv"
file_index_in = "F:/封城数据处理/封城数据/main_deal/data/西安/610100_西安_move_in.csv"
...
#西安 封城时间 20211223 20220115  比较时间2021/12/09 -2022/1/31(接近春节) 阈值选取为0.1 确定！
...
# 根据路径画图
def drawpicture(filePath,threshold):
    """
    输入文件路径最后绘制成图G
    """
    global dataMiga
...
#做取样标本，对in和out完全合并 第一步
def merge_in_out_2021(deal_time):
    """
    属于第一步
    合并2021年的整合数据
    :return:
    ...
    # 开始写入整理完的数据csv
    move_in_data = pd.read_csv(fileNameFront + "in/"+deal_time+".csv")
    move_out_data = pd.read_csv(fileNameFront + "out/"+deal_time+".csv")
    with open("F:/封城数据处理/封城数据/main_deal/data/"+deal_time+"_HaveRepeat.csv", 'w',
              encoding="utf-8", newline='') as csvfile:
        writer = csv.DictWriter(csvfile, field_order_move_in)
        writer.writeheader()
        for row_in in tqdm(move_in_data.iterrows(),desc="第一步in和out合并",total=len(move_in_data)):
            ...
            for row_out in move_out_data.iterrows():
                ...
                if compare_str_one == compare_str_two:
                    ...
                    writer.writerow(row)
                    break
#做取样标本，对in和out完全合并 第二步
def deal_merge_data_onefile(deal_time):
    """
    属于第二步
    将in和out合并完的数据继续进行合并
    :return:
    ...
    field_order_move_in = ["city_name", 'city_id_name', 'num']
    # 开始写入整理完的数据csv
    need_deal_file_one = pd.read_csv("F:/封城数据处理/封城数据/main_deal/data/"+deal_time+"_HaveRepeat.csv")
    need_deal_file_two = pd.read_csv("F:/封城数据处理/封城数据/main_deal/data/"+deal_time+"_HaveRepeat.csv")

    with open("F:/封城数据处理/封城数据/main_deal/data/"+deal_time+"_merge.csv", 'w',
                  encoding="utf-8", newline='') as csvfile:
        writer = csv.DictWriter(csvfile, field_order_move_in)
        writer.writeheader()
        for row_one in tqdm(need_deal_file_one.iterrows(),desc="第二步 处理重复条数",total=len(need_deal_file_one)):
            list_a=[]
            ...
            for row_two in need_deal_file_two.iterrows():
                ...
                if compare_two_str(list_a,list_b):
                    ...
                    break
# 最终确定自我中心城市
def get_around_city(city_list,threshold):
    """
    找到自我中心网络的城市
    输入城市列表寻找他的高一阶城市
    :return:
    """
    G = drawpicture(reference_file,threshold)
    print("点：",len(G.nodes()))
    print("边：",len(G.edges()))
    edge_finall= []
...
def merge_index_city(file_index_in,file_index_out,final_file_adress):
    """
    合并in和out的迁徙指数，用来寻找阈值城市
    :param file_index_in:
    :param file_index_out:
    :param final_file_adress:
    :return:
    """
    # 开始写入整理完的数据csv
    index_in = pd.read_csv(file_index_in)
    index_out = pd.read_csv(file_index_out)

    field_order_move_in = ["date","index"]
    with open(final_file_adress, 'w',
              encoding="utf-8", newline='') as csvfile:
        writer = csv.DictWriter(csvfile, field_order_move_in)
        writer.writeheader()
        for row_one in tqdm(index_in.iterrows(), desc="处理合并开始", total=len(index_in)):
            city_name_one = row_one[1]["date"]
            city_name_two = row_one[1]["index"]
            for row_two in index_out.iterrows():
                city_name_three = row_two[1]["date"]
                city_name_four = row_two[1]["index"]
                if str(city_name_one) == str(city_name_three):
                    valueColThree = (float(city_name_two) + float(city_name_four)) / 2
                    row = {"date": city_name_one, "index": valueColThree}
                    writer.writerow(row)
                    break

if __name__ == '__main__':
    #一阶城市
    # get_around_city(["西安"],0.1)
    # 二阶城市
    # get_around_city(First_order_xian,0.1)
    # 三阶城市
    # get_around_city(Second_order_xian,0.1)
    # 四阶城市
    # get_around_city(Third_order_xian,0.1)
    # 五阶城市
    get_around_city(five_order_xian,0.1)


    #取样标本某一天的网络
    # merge_in_out_2021("20211210")
    # deal_merge_data_onefile("20211210")


    #合并迁徙指数
    # merge_index_city(file_index_in,file_index_out,"F:/封城数据处理/封城数据/main_deal/data/西安/610100_西安.csv")
